    ...
    <Dialog open={isOpen} onOpenChange={open => { if (!open) handleClose(); }}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Enter the password for <strong>{folderName}</strong> to proceed with the{' '}
            <strong>{actionLabel}</strong> action.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-2">
          {isLockedOut ? (
            <Alert variant="destructive">
              ...
              <AlertDescription>
                This folder action is locked for this session due to too many failed password
                attempts.
              </AlertDescription>
            </Alert>
          ) : (
