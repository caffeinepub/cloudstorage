...
import { useVerifyFolderPassword, useAddNotification } from '@/hooks/useQueries';
...
function lockOutFolder(folderId: string) {
  ...
  if (!current.includes(folderId)) {
    sessionStorage.setItem(SESSION_LOCKOUT_KEY, JSON.stringify([...current, folderId]));
  }
}
...
export default function FolderPasswordPrompt({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const success = await verifyMutation.mutateAsync({ folderId, attempt: hashed });
      ...
      if (success) {
        ...
        if (newAttempts >= MAX_ATTEMPTS) {
          ...
          // Create in-app notification for suspicious access
          if (identity) {
            try {
              await addNotificationMutation.mutateAsync({
                ...
                notificationType: {
                  ...
                  activityAlert: {
                    activityType: 'Suspicious Access Attempt',
                    fileName: folderName,
                    fileId: folderId,
        ...
        <div className="space-y-4 py-2">
          {isLockedOut ? (
            <div className="flex items-start gap-3 rounded-lg border border-destructive/30 bg-destructive/10 p-4">
              ...
              <div>
                <p className="text-sm font-medium text-destructive">Access Locked</p>
                <p className="text-xs text-muted-foreground mt-1">
                  Too many failed attempts. This folder is locked for the current session. A
                  security notification has been sent to your account.
                </p>
              </div>
            </div>
          ) : (
            <>
              ...
              {attempts > 0 && (
                <div className="flex items-center gap-2 text-sm text-amber-600 dark:text-amber-400">
                  ...
                  <span>
                    Incorrect password.{' '}
                    <strong>{remaining} attempt{remaining !== 1 ? 's' : ''}</strong> remaining.
                  </span>
                </div>
              )}
            </>
