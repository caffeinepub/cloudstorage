...
export default function RestoreDialog({
  ...
  const itemTypeLabel = hasFiles && hasFolders ? 'item' : hasFolders ? 'folder' : 'file';

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Restore {itemTypeLabel.charAt(0).toUpperCase() + itemTypeLabel.slice(1)}s</DialogTitle>
          <DialogDescription>
            Choose where to restore {selectedItems.length} selected {itemTypeLabel}(s)
        ...
        <DialogFooter>
          ...
          <Button onClick={handleRestore}>Restore {itemTypeLabel.charAt(0).toUpperCase() + itemTypeLabel.slice(1)}s</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
