...
import { Pin, FileText, AlertTriangle } from 'lucide-react';
  ...
  if (isLoading) {
    return (
      <div className="flex gap-3 overflow-x-auto pb-1">
        {[1, 2, 3].map((i) => (
          <div key={i} className="shrink-0 w-36 rounded-xl border border-border bg-card p-3 space-y-2">
  ...
  if (!favorites || favorites.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-6 text-center">
        ...
        <p className="text-xs text-muted-foreground mt-1">Star files to pin them here</p>
      </div>
    );
  }
    ...
    <div className="flex gap-3 overflow-x-auto pb-1">
      {favorites.map((fav) => (
        ...
        >
          ...
          <p className="text-xs font-medium text-foreground truncate">{fav.fileName}</p>
          <p className="text-xs text-muted-foreground">{formatBytes(fav.size)}</p>
        </div>
      ))}
    </div>
  );
