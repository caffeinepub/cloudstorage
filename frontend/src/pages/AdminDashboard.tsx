...
export default function AdminDashboard() {
  ...
  const { data: adminTableData, isLoading: tableDataLoading } = useGetAdministrationsTableData();
  const { data: loginLogs, isLoading: loginLogsLoading } = useGetLoginLogTable();
  const setApprovalMutation = useSetApproval();

  // The current user's principal string (normalized)
  const currentPrincipal = identity?.getPrincipal().toString().trim().toLowerCase() ?? '';
  ...
  const handleApprove = (principalStr: string) => {
    setApprovalMutation.mutate(
      { user: Principal.fromText(principalStr), status: ApprovalStatus.approved },
      {
  ...
  const handleReject = (principalStr: string) => {
    setApprovalMutation.mutate(
      { user: Principal.fromText(principalStr), status: ApprovalStatus.rejected },
      {
    ...
    <div className="container mx-auto p-6 max-w-7xl">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600 dark:text-green-400">
              {approvalsLoading
                ? '-'
                : (approvals?.filter((a) => a.status === ApprovalStatus.approved).length ?? 0)}
            </div>
      ...
      <Tabs defaultValue="user-management">
        <TabsList className="mb-6">
          ...
          <TabsTrigger value="storage-management">Storage Management</TabsTrigger>
        ...
        <TabsContent value="user-management" className="space-y-6">
                  ...
                  <TableBody>
                    {pendingApprovals.map((approval) => {
                      ...
                      const isProcessing =
                        ...
                        setApprovalMutation.variables?.user.toString() === principalStr;
                      ...
                      return (
                        <TableRow key={principalStr}>
                          ...
                          <TableCell>
                            {isAdminRow ? (
                              <Badge className="bg-blue-100 text-blue-800 border-blue-200 dark:bg-blue-900/30 dark:text-blue-300 dark:border-blue-800" variant="outline">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Complete list of all user registration requests</CardDescription>
            </CardHeader>
            <CardContent>
              {approvalsLoading ? (
                <div className="space-y-3">
                        ...
                        <TableRow key={principalStr}>
                          ...
                          <TableCell className="text-right">
                            <div className="flex items-center justify-end gap-2">
                              ...
                              {approval.status !== ApprovalStatus.rejected && !isAdminRow && (
                                <Button
                                  size="sm"
                                  variant="outline"
                                  className="text-destructive border-destructive/30 hover:bg-destructive/10"
            ...
            <CardContent>
              {loginLogsLoading ? (
                ...
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <p className="text-sm font-medium text-foreground">No login activity yet</p>
                  <p className="text-xs text-muted-foreground mt-1">
