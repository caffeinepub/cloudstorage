...
import { Clock, XCircle, LogOut, RefreshCw, HardDrive } from 'lucide-react';
...
export default function WaitingApproval() {
  ...
  // requestApprovalMutation is stable (from useMutation), actor/actorFetching/identity are the real deps
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-background">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-muted-foreground text-sm">Checking approval status...</p>
        </div>
      </div>
    );
  }
      ...
      <Card className="w-full max-w-md shadow-lg border-border">
        <CardHeader className="text-center pb-4">
          ...
          <CardDescription className="text-sm mt-2 leading-relaxed">
            {isPending && !isRejected ? (
              <>
                Your registration request has been submitted and is awaiting review by an
                administrator. You will gain access once your account is approved.
              </>
            ) : (
              <>
                Unfortunately, your registration request was not approved. Please contact an
          ...
          {isPending && !isRejected && (
            <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-3 text-sm text-amber-800 dark:text-amber-300">
              <div className="flex items-start gap-2">
                ...
                <span>
                  Your request is in the queue. An admin will review it shortly. You can check back
                  later by refreshing this page.
                </span>
              </div>
            </div>
          )}
