import type { Folder, FolderProtection } from "@/backend";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export default function FolderProtectionModal({
  ...
  const handleSetPassword = async () => {
    ...
    if (hasExistingProtection) {
      ...
      if (!isCurrentPasswordVerified) {
        setCurrentPasswordError(
          "Incorrect current password. Please verify before proceeding.",
        );
        return;
      }
  ...
  const handleRemovePassword = () => {
    ...
    if (!currentPassword) {
      setCurrentPasswordError(
        "Please enter your current password to remove protection",
      );
      return;
    }
    if (!isCurrentPasswordVerified) {
      setCurrentPasswordError(
        "Incorrect current password. Please verify before removing protection.",
      );
      return;
    }
  ...
  const handleToggleLock = async (checked: boolean) => {
    ...
    if (hasExistingProtection && protection?.isLocked) {
      if (!currentPassword) {
        setCurrentPasswordError(
          "Please enter your current password to change the lock state",
        );
        return;
      }
      if (!isCurrentPasswordVerified) {
        setCurrentPasswordError(
          "Incorrect current password. Please verify before toggling the lock.",
        );
        return;
      }
    }
