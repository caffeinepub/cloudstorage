import { useListAllUsersStorage, useGetUserProfile, useSetUserQuota } from '../hooks/useQueries';
...
export default function UserStorageManager() {
  ...
  const handleSetQuota = async (userPrincipal: string) => {
    ...
    try {
      ...
      await setUserQuota.mutateAsync({ userPrincipal, quota: quotaBytes });
...
function UserCell({ principal }: { principal: any }) {
