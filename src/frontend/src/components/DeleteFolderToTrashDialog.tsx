...
export default function DeleteFolderToTrashDialog({
  ...
  const handleConfirm = async () => {
    ...
    setIsDeleting(true);
    try {
      const days = Number.parseInt(retentionDays);
        ...
        <div className="space-y-4 py-4">
          <div className="bg-amber-50 dark:bg-amber-950 p-3 rounded-md flex items-start gap-2">
            ...
            <p className="text-sm text-amber-900 dark:text-amber-100">
              All files and subfolders within this folder will be moved to Trash
              and can be restored from the Trash page before the retention
              period expires.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 p-3 bg-muted rounded-md">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-muted-foreground">
                Created{" "}
                {new Date(
                  Number(folder.createdAt) / 1000000,
                ).toLocaleDateString()}
              </p>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <p className="text-xs text-muted-foreground">
              Choose how long the folder will be kept in Trash before automatic
              deletion
            </p>
          </div>
        </div>
