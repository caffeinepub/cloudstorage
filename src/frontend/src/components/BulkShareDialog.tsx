...
export default function BulkShareDialog({
  ...
  const handleShare = async () => {
    ...
    if (failed === 0) {
      toast.success(
        `Successfully shared ${succeeded} file${succeeded !== 1 ? "s" : ""}`,
      );
    } else if (succeeded === 0) {
      toast.error(
        `Failed to share all ${failed} file${failed !== 1 ? "s" : ""}`,
      );
    } else {
      toast.warning(
        `Shared ${succeeded} file${succeeded !== 1 ? "s" : ""}, ${failed} failed`,
      );
    }
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Share {selectedFileIds.length} File
            {selectedFileIds.length !== 1 ? "s" : ""}
          </DialogTitle>
          <DialogDescription>
            Share all selected files with another user. Enter their principal ID
            and set permissions.
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  disabled={isSharing}
                />
                <Label htmlFor="canDownload" className="font-normal">
                  Can Download
                </Label>
              </div>
            </div>
