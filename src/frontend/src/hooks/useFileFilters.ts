  ...
  const filterFiles = useMemo(() => {
    return (files: FileMetadata[]): FileMetadata[] => {
      return files.filter((file) => {
        // Name filter
        if (filters.nameFilter && !file.name.toLowerCase().includes(filters.nameFilter.toLowerCase())) {
          return false;
        }
        ...
        if (filters.dateRange.start !== null && fileDate < filters.dateRange.start) {
          return false;
        }
        if (filters.dateRange.end !== null && fileDate > filters.dateRange.end) {
          return false;
        }

        // Size range filter (in bytes)
        const fileSize = Number(file.size);
        if (filters.sizeRange.min !== null && fileSize < filters.sizeRange.min) {
          return false;
        }
        if (filters.sizeRange.max !== null && fileSize > filters.sizeRange.max) {
          return false;
        }
