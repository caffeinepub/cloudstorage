...
const RecentUploadsContext = createContext<
  RecentUploadsContextType | undefined
>(undefined);
...
export function RecentUploadsProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <RecentUploadsContext.Provider
      value={{ recentUploads, addRecentUpload, clearRecentUploads }}
    >
      {children}
    </RecentUploadsContext.Provider>
  );
...
export function useRecentUploads() {
  ...
  if (context === undefined) {
    throw new Error(
      "useRecentUploads must be used within a RecentUploadsProvider",
    );
  }
  return context;
}
